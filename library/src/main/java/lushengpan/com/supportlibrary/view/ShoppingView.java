...
public class ShoppingView extends View {
    ...
    private void init(AttributeSet attrs) {
        ...
        //展示文案大小
        textSize = (int) typeArray.getDimension(R.styleable.ShoppingView_sv_text_size, sp2px(13));

        //背景色
        int bgColor = typeArray.getColor(R.styleable.ShoppingView_sv_bg_color, ContextCompat.getColor(getContext(), R.color.transparent));
        typeArray.recycle();
        ...
        mPaintTextbg.setStrokeWidth(sp2px(1));
        mPaintTextbg.setAntiAlias(true);


        mPaintBg1 = new Paint();
        mPaintBg1.setColor(Color.WHITE);
        mPaintBg1.setStyle(Paint.Style.FILL);
        mPaintBg1.setAntiAlias(true);


        mPaintMinus = new Paint();
        mPaintMinus.setColor(bgColor);
        mPaintMinus.setStyle(Paint.Style.FILL_AND_STROKE);
        mPaintMinus.setAntiAlias(true);
        mPaintMinus.setStrokeWidth(textSize / 5);

        mPaintMinus1 = new Paint();
        mPaintMinus1.setColor(bgColor);
        mPaintMinus1.setStyle(Paint.Style.STROKE);
        mPaintMinus1.setAntiAlias(true);
        mPaintMinus1.setStrokeWidth(textSize / 6);

        mPaintText = new Paint();
        mPaintText.setColor(Color.WHITE);
        mPaintText.setStrokeWidth(sp2px(1));
    ...
    private void drawBgMove(Canvas canvas) {
        ...
        canvas.drawRoundRect(new RectF(MAX_WIDTH - MAX_HEIGHT / 2 - MAX_HEIGHT / 2, 0, MAX_WIDTH, MAX_HEIGHT), 8, 8, mPaintBg);
        canvas.drawRoundRect(new RectF(MAX_WIDTH - MAX_HEIGHT / 2 + sp2px(1) - MAX_HEIGHT / 2, 0 + sp2px(1), MAX_WIDTH - sp2px(1), MAX_HEIGHT - sp2px(1)), 8, 8, mPaintText);

        //      canvas.drawRect(new RectF(MAX_WIDTH - MAX_HEIGHT / 2 - MAX_HEIGHT / 2, 0, MAX_WIDTH, MAX_HEIGHT), mPaintMinus);
        //    canvas.drawRect(new RectF(MAX_WIDTH - MAX_HEIGHT + sp2px(1), 0 + textSize / 5, MAX_WIDTH - MAX_HEIGHT + sp2px(2), MAX_HEIGHT - textSize / 5), mPaintBg1);


//        Bitmap mBitmap = BitmapFactory.decodeResource(getResources(),R.mipmap.youku);
//        canvas.drawBitmap(mBitmap,null,new RectF(MAX_WIDTH - MAX_HEIGHT / 2 - MAX_HEIGHT / 2, 0, MAX_WIDTH - MAX_HEIGHT / 2 + MAX_HEIGHT / 2, MAX_HEIGHT),mPaintText);
        //  canvas.drawRect(new RectF(mWidth + MAX_HEIGHT / 2, 0, MAX_WIDTH - MAX_HEIGHT / 2, MAX_HEIGHT), mPaintMinus);
        //      canvas.drawRoundRect(new RectF(MAX_WIDTH - MAX_HEIGHT / 2 - MAX_HEIGHT / 2, MAX_HEIGHT / 2 - MAX_HEIGHT / 2, MAX_WIDTH - MAX_HEIGHT / 2 + MAX_HEIGHT / 2, MAX_HEIGHT / 2 + MAX_HEIGHT / 2), 5, 5, mPaintMinus);
    }
    ...
    private void drawShoppingText(Canvas canvas) {
        canvas.drawLine(MAX_WIDTH - MAX_HEIGHT / 2, MAX_HEIGHT / 4, MAX_WIDTH - MAX_HEIGHT / 2, MAX_HEIGHT / 4 * 3, mPaintTextbg);
        canvas.drawLine(MAX_WIDTH - MAX_HEIGHT / 2 - MAX_HEIGHT / 4, MAX_HEIGHT / 2, MAX_WIDTH - MAX_HEIGHT / 4, MAX_HEIGHT / 2, mPaintTextbg);
        //       canvas.drawText(mShoppingText, (MAX_WIDTH / 2 - (mWidth + MAX_HEIGHT / 2)) / 2 + (mWidth + MAX_HEIGHT / 2) - getTextWidth(mPaintText, mShoppingText) / 2f, MAX_HEIGHT / 2 + getTextHeight(mShoppingText, mPaintText) / 2f, mPaintText);
    }
    ...
    private void drawAddBtn(Canvas canvas) {
        ...
        canvas.drawRoundRect(new RectF(MAX_WIDTH - MAX_HEIGHT / 2 - MAX_HEIGHT / 2, MAX_HEIGHT / 2 - MAX_HEIGHT / 2, MAX_WIDTH - MAX_HEIGHT / 2 + MAX_HEIGHT / 2, MAX_HEIGHT / 2 + MAX_HEIGHT / 2), 8, 8, mPaintBg);
//        canvas.drawCircle(MAX_WIDTH - MAX_HEIGHT / 2, MAX_HEIGHT / 2, MAX_HEIGHT / 2, mPaintBg);
//        canvas.drawRect(new RectF(mWidth + MAX_HEIGHT / 2, 0, MAX_WIDTH / 2, MAX_HEIGHT), mPaintBg);
    ...
    private void drawMinusBtn(Canvas canvas, float angle) {
        ...
        canvas.drawRoundRect(new RectF(mMinusBtnPosition - MAX_HEIGHT / 2, 0, mMinusBtnPosition + MAX_HEIGHT / 2, MAX_HEIGHT), 8, 8, mPaintBg);
        canvas.drawRoundRect(new RectF(mMinusBtnPosition - MAX_HEIGHT / 2 + sp2px(1), 0 + sp2px(1), mMinusBtnPosition + MAX_HEIGHT / 2 - sp2px(1), MAX_HEIGHT - sp2px(1)), 8, 8, mPaintText);
        canvas.drawLine(mMinusBtnPosition - MAX_HEIGHT / 4, MAX_HEIGHT / 2, mMinusBtnPosition + MAX_HEIGHT / 4, MAX_HEIGHT / 2, mPaintTextbg);
        if (angle != 0) {
            canvas.rotate(-angle, mMinusBtnPosition, MAX_HEIGHT / 2);
        }
    }
